...
fix32 player_y = FIX32(50);
fix32 player_vel = FIX32(3.5);

Sprite *boss;
fix32 boss_x = FIX32(250);
fix32 boss_y = FIX32(100);
...
//Estou utilizando as mesmas variaveis da aula de paralax, preguica
int main()
{
    SPR_init();
    ...
    PAL_setPalette(PAL3, boss_img.palette->data, DMA);

    //Aumentando o tamanho do VDP plane, o tamanho do plano que fica fora da camera ate comecar a se repetir
    //Bom para cutscenes iniciais da tela de abertura
    //VDP_setPlaneSize(128, 32, TRUE);

    //Carregando o tileset na vram
    VDP_loadTileSet(&shinobi_foreground_tileset, ind, DMA);

    //Aparentemente o SGDK ja identificou a cor de transparencia
    ...
    boss = SPR_addSprite(&boss_img, F32_toInt(boss_x), F32_toInt(boss_y), TILE_ATTR(PAL3, FALSE, FALSE, FALSE));
...
static void camera()
{
    ...
    setEnemyPosition(boss, boss_x, boss_y);
}

//Ta, para o inimigo ser posicionado no lugar correto, tu precisa subtrair pela new_camera
//O problema eh que depois de um certo tempo o inimigo vai aparecer de novo na tela se tu continuar andando
//Entao para resolver isso tu simplesmente deixa o inimigo invisivel assim que ele sai da tela
static void setEnemyPosition(Sprite *enemy_sprite, fix32 enemy_x, fix32 enemy_y)
{
    s16 enemy_x_s16 = F32_toInt(enemy_x) - new_camera_x;
    s16 enemy_y_s16 = F32_toInt(enemy_y) - new_camera_y;

    if (enemy_x_s16 < -64 || enemy_x_s16 > HORIZONTAL_RESOLUTION || enemy_y_s16 < -64 || enemy_y_s16 > VERTICAL_RESOLUTION)
    {
        SPR_setVisibility(boss, HIDDEN);
    }
    else
    {
        SPR_setVisibility(boss, VISIBLE);
    }

    SPR_setPosition(enemy_sprite, enemy_x_s16, enemy_y_s16);
}